...
def evaluate():
    ...
    print(f"\nTotal in ground truth: {total_evaluated}")
    print(f"✅ Correct: {correct} ({correct/decided*100:.1f}% of decided)")
    print(f"❌ Incorrect: {incorrect} ({incorrect/decided*100:.1f}% of decided)")
    print(f"⚠️  Sent to human: {not_decided} ({not_decided/total_evaluated*100:.1f}%)")
    
    if decided > 0:
        print(f"\n📊 Accuracy (on decided cases): {correct}/{decided} = {correct/decided*100:.1f}%")
